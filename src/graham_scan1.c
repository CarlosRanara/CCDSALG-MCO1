/**
 * graham_scan1.c
 * Implementation of Graham's Scan algorithm using bubble sort (slow version)
 * Group 16 Assignment: Bubble Sort (slow algorithm)
...
void grahamScanSlow(Point points[], int n, Point hull[], int *hullSize) {
    ...
    /* Sort points by polar angle with respect to anchor point using bubble sort */
    bubbleSortPoints(pointsCopy, n, nAnchorIndex);
    ...
    printf("Graham's Scan (Bubble Sort) execution time: %.2f milliseconds\n", tElapsed);
}