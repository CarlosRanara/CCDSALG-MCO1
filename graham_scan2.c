/**
 * graham_scan2.c
 * Implementation of Graham's Scan algorithm using quick sort (fast version)
 */
...
void grahamScanFast(Point points[], int n, Point hull[], int *hullSize) {
    ...
    /* Sort points by polar angle with respect to anchor point using quick sort */
    quickSortPoints(pointsCopy, n, nAnchorIndex);
    ...
    printf("Graham's Scan (Quick Sort) execution time: %.2f milliseconds\n", tElapsed);
}